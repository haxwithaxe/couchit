...
__version_info__ = (1, 0, 1, 11, 'a') # first three nums match Markdown.pl
__version__ = '1.0.1.11a'
...
from couchit.utils import smart_str
...
class Markdown(object):
    ...
    def __init__(self, html4tags=False, tab_width=4, safe_mode=None,
                 extras=None, link_patterns=None, use_file_vars=False, codehilite=None):
        if html4tags:
    ...
    def _code_block_sub(self, match):
        ...
        if 'codehilite' in self.extras:
            code = CodeHilite(codeblock, linenos=self.codehilite.get('force_linenos'),
                              css_class=self.codehilite.get('css_class','codehilite'))
            return "\n\n%s\n\n" % code.hilite()
    ...
    def _do_link_patterns(self, text):
        ...
        """
        link_from_hash = {}
        for regex, href, group in self.link_patterns:
            replacements = []
            for match in regex.finditer(text):
                replacements.append((match.span(), match.span(group), match.expand(href)))
            for (st, et), (start, end), href in reversed(replacements):
                escaped_href = (
                ...
                hash = md5(smart_str(link)).hexdigest()
                link_from_hash[hash] = link
                text = text[:st] + hash + text[et:]
